...
function appendNewCard(parentElement) {
  ...
  newCard.innerHTML = `<div class="card-down"></div>
                      <div class="card-up"></div>`;
...
function shuffleCardImageClasses() {
  ...
  let imageCollection = ["image-1", "image-1", "image-2", "image-2", "image-3", "image-3", "image-4", "image-4", "image-5", "image-5", "image-6", "image-6"];
  ...
  let result = shuffle(imageCollection);
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++){
    ...
    let cardImageClass = shuffledImageClasses[i];
    let cardIndex = i;
    let newCard = { index: cardIndex,
                     element: card,
                     imageClass: cardImageClass};
    cards.push(newCard);
  }
...
function incrementCounter(counterName, parentElement) {
  ...
  if ( counterName == "flip-count" ) {
    document.getElementById("flip-count").innerHTML = counters[counterName];
  }
  if ( counterName == "match-count" ) {
    document.getElementById("match-count").innerHTML = counters[counterName];
  }
...
function flipCardWhenClicked(cardObject) {
  ...
  cardObject.element.onclick = function() {
    ...
    cardObject.element.classList.add(cardObject.imageClass);
...
function onCardFlipped(newlyFlippedCard) {
  ...
  incrementCounter("flip-count", "flip-count");
  ...
  incrementCounter("match-count", "match-count");
  matchAudio.play();
